...
function MongodbSycner(options) {
  ...
  this._storeUrl = 'https://www.mongodb.org/dl';
}
...
proto.listdir = function* (fullname, dirIndex) {
  ...
  var items = [];
  for (var i = 0, len = mongoPlatforms.length; i < len; i++) {
    var platform = mongoPlatforms[i];
    var platformUri = this._storeUrl + '/' + platform;
