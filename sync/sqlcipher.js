...
proto.listdiff = function* listdiff(fullname, dirIndex) {
  ...
  for (var i = 0; i < needs.length; i++) {
    ...
    if (pkg.binary && pkg.binary.napi_versions) {
      for (const napiVersion of pkg.binary.napi_versions) {
        ...
        for (const nodePlatformAndArch of nodePlatformAndArchs) {
          ...
          items.push({
            ...
            type: 'file',
            parent: fileParent,
            downloadURL: this.disturl + fileParent + name,
            name: name,
          });
        }
      }
...
proto.formatDownloadItem = function(fileParent, pkg, nodeAbiVersion, nodePlatform) {
  ...
  // https://journeyapps-node-binary.s3.amazonaws.com/@journeyapps/sqlcipher/v5.0.0/napi-v3-linux-x64.tar.gz
  // https://journeyapps-node-binary.s3.amazonaws.com/@journeyapps/sqlcipher/v5.0.0/napi-v3-linux-arm64.tar.gz
  // https://journeyapps-node-binary.s3.amazonaws.com/@journeyapps/sqlcipher/v5.0.0/napi-v3-darwin-x64.tar.gz
  // https://journeyapps-node-binary.s3.amazonaws.com/@journeyapps/sqlcipher/v5.0.0/napi-v3-darwin-arm64.tar.gz
  // https://journeyapps-node-binary.s3.amazonaws.com/@journeyapps/sqlcipher/v5.0.0/napi-v3-win32-x64.tar.gz
  ...
  // https://journeyapps-node-binary.s3.amazonaws.com/@journeyapps/sqlcipher/v5.3.0/electron-v7.3-darwin-arm64.tar.gz
